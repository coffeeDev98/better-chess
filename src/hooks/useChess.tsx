...
import { ChessInstance, Move, Piece, PieceType, ShortMove, Square } from 'chess.js';
import { Color, Dests, Key, MoveMetadata, Role, SetPremoveMetadata, Elements } from 'chessground/types';
...
const useChess = (Agora: any, Multiplayer: any) => {
	...
	useEffect(
		() => {
			console.log('FEN: ', Object.fromEntries(new URLSearchParams(window.location.search).entries()).fen);
			if (chess && Object.fromEntries(new URLSearchParams(window.location.search).entries()).fen) {
				chess.load(Object.fromEntries(new URLSearchParams(window.location.search).entries()).fen);
				game.loadFen(Object.fromEntries(new URLSearchParams(window.location.search).entries()).fen);
				setState({
					...state,
					fen: Object.fromEntries(new URLSearchParams(window.location.search).entries()).fen
				});
			}
		},
		[chess]
	...
	useEffect(
		() => {
			console.log('STATE: ', state, chess.in_check(), chess.turn());
	...
	useEffect(
		() => {
			...
			console.log("MULTIPLAYER_AGORA: ", Agora);
			if (Agora.channel) {
				Agora.channel.on('ChannelMessage', (message: any) => {
					const data = parseJSON(message.text);
					// console.log("MESSAGE DATA: ", data);
					console.log("MULTIPLAYER_RECEIVE: ", message);
					switch (data.type) {
						case BOARD_MOVE_UPDATE:
							onDrop({
								detail: {
									source: data.json.move.from,
									target: data.json.move.to
								}
							});
							break;
	...
	const highlightSquare = (sourceSquare: any, squaresToHighlight: string[]) => {
		const highlightStyles = [sourceSquare, ...squaresToHighlight].reduce((a, c) => {
			return {
				...
				...{
					[c]: {
						background: 'radial-gradient(circle, rgb(123,97,255,0.5) 20%, transparent 0%)',
						borderRadius: '50%'
					}
				},
				...squareStyling({
	...
	const inCheck = (turn: string) => {
		...
		console.log(
			'INCHECK: ',
			Object.keys(state.boardPosition).find((key: string) => state.boardPosition[key] === `${turn}K`)
		);
		setState({
			...state,
			squareStyles: {
	...
	const onMouseOverSquare = ({ square }: { square: any }) => {
		console.log("MOUSEOVER_SQUARE")
	...
	const onMouseOutSquare = ({ square }: { square: any }) => {
		removeHighlightSquare();
	};

	const promotion = (sourceSquare: any, targetSquare: any, e?: Exclude<any, 'p' | 'k'>) => {
	...
	const onDrop = ({ detail }: { detail: any }) => {
		const { source, target, piece, newPosition, oldPosition, orientation, setAction } = detail;
...
const squareStyling = ({ pieceSquare, history }: { pieceSquare: any; history: any }) => {
